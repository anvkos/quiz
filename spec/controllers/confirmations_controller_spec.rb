...
RSpec.describe ConfirmationsController, type: :controller do
  describe 'PATCH #email' do
    context 'with valid attributes' do
      it 'assigns @user by token' do
        user = create(:user, :unconfirmed)
        patch :email, params: { user: { email: 'user@test.com' }, token: user.confirmation_token }
      ...
      it 'set unconfirmed email' do
        ...
        patch :email, params: { user: { email: confirmed_email }, token: user.confirmation_token }
        user.reload
    ...
    context 'user try confirm email with invalid attributes' do
      context 'email alredy use' do
        it 'try use email another user' do
          ...
          user = create(:user, :unconfirmed)
          patch :email, params: { user: { email: another_user_email }, token: user.confirmation_token }
      ...
      context 'email empty' do
        it 'try use email blank' do
          user = create(:user, :unconfirmed)
          patch :email, params: { user: { email: '' }, token: user.confirmation_token }
